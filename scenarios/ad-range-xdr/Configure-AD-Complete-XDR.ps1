...
# Configure-AD-Complete-XDR.ps1
# Complete Active Directory Configuration for Range XDR Attack Simulation
# Execute on DC01 (RangeXDR-AD01) as Enterprise Administrator
...
Write-Host "Complete AD Configuration - Range XDR" -ForegroundColor Cyan
Write-Host "Domain: xdr.ad.bakerstreetlabs.io" -ForegroundColor Yellow
Write-Host "Version: 1.0" -ForegroundColor Gray
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Configuration variables - RANGE XDR
$domainName = "xdr.ad.bakerstreetlabs.io"
$domainDN = "DC=xdr,DC=ad,DC=bakerstreetlabs,DC=io"
...
try {
    ...
    if ($domain.DNSRoot -ne $domainName) {
        Write-Host "[!] WARNING: Current domain ($($domain.DNSRoot)) does not match expected ($domainName)" -ForegroundColor Yellow
        Write-Host "[*] Continuing with current domain..." -ForegroundColor Yellow
    ...
    Write-Host "[!] ERROR: Could not verify domain - $_" -ForegroundColor Red
    exit 1
}
Write-Host ""
...
try {
    Get-ADComputer -Filter {Name -like "*XDR*" -and (Name -like "*CA*" -or Name -like "*AD01*")} | ForEach-Object {
        try {
...
Write-Host "  2. Test insider account login: xdr\\insider" -ForegroundColor White
